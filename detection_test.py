...
predicted_dir_path = cfg.TEST.PREDICTED_DIR_PATH
ground_truth_dir_path = cfg.TEST.GT_DIR_PATH
...
if '.weights' in cfg.TEST.LOAD_WEIGHTS_PATH:
    utils.load_weights(model, cfg.TEST.LOAD_WEIGHTS_PATH)
else:
    model.load_weights(cfg.TEST.LOAD_WEIGHTS_PATH)
...
#        bbox_data_gt = np.array([list(map(int, box.split(','))) for box in annotation[1:]])
        bbox_data_gt = np.array([list(map(lambda x: int(float(x)), box.split(','))) for box in annotation[1:]])
